...
function App() {
  ...
  // Create binary counter circuit
  const addBinaryCounter = useCallback(() => {
    const baseX = 250;
    const baseY = 300;
      const newNodes = [
      // Clock input
      ...
      // Binary display for counter value
      {
        id: generateNodeId('binaryDisplay'),
        type: 'binaryDisplay',
        position: { x: baseX + 200, y: baseY },
        data: { label: 'Counter', bitWidth: 4 },
      },
      // Multi-LED for visual output
      {
        id: generateNodeId('multiLED'),
        type: 'multiLED',
        position: { x: baseX + 400, y: baseY },
        data: { label: 'LED Array', bitWidth: 4 },
      }
    ];
    
    const newEdges = [
      { id: generateEdgeId(newNodes[2].id, newNodes[3].id, 'out', 'in'), source: newNodes[2].id, target: newNodes[3].id, sourceHandle: 'out', targetHandle: 'in' }
    ];
  ...
  const addMultiplexer = useCallback(() => {
    ...
    setNodes(prevNodes => [...prevNodes, ...newNodes]);
    setEdges(prevEdges => [...prevEdges, ...newEdges]);  }, [setNodes, setEdges]);

  // ALU Circuit
  const addALU = useCallback(() => {
    const baseX = 250;
    const baseY = 200;
    const newNodes = [
      // Input A switches (4-bit)
      ...Array.from({ length: 4 }, (_, i) => ({
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + (i * 40) },
        data: { label: `A${3-i}`, state: false },
      })),
      // Input B switches (4-bit)
      ...Array.from({ length: 4 }, (_, i) => ({
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 100, y: baseY + (i * 40) },
        data: { label: `B${3-i}`, state: false },
      })),
      // Operation select switches (3-bit for 8 operations)
      ...Array.from({ length: 3 }, (_, i) => ({
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX, y: baseY + 200 + (i * 40) },
        data: { label: `Op${2-i}`, state: false },
      })),
      // Output LEDs (4-bit result)
      ...Array.from({ length: 4 }, (_, i) => ({
        id: generateNodeId('led'),
        type: 'led',
        position: { x: baseX + 300, y: baseY + (i * 40) },
        data: { label: `Result${3-i}`, state: false },
      })),
      // ALU flag LEDs
      {
        id: generateNodeId('led'),
      ...
      },
      {
        id: generateNodeId('led'),
        type: 'led',
        position: { x: baseX + 400, y: baseY + 80 },
        data: { label: 'Overflow', state: false },
      }
    ];
  ...
  const add7SegmentDecoder = useCallback(() => {
    const baseX = 300;
    const baseY = 200;
    const newNodes = [
      // 4-bit input switches (BCD input)
      ...Array.from({ length: 4 }, (_, i) => ({
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + (i * 40) },
        data: { label: `BCD${3-i}`, state: false },
      })),
      // 3:8 Decoder for BCD to 7-segment conversion
      {
        id: generateNodeId('binaryDecoder'),
        type: 'binaryDecoder',
        position: { x: baseX, y: baseY + 60 },
        data: { label: '7-Seg Decoder', inputBits: 4 },
      },
      // 7 output LEDs representing segments (a, b, c, d, e, f, g)
      ...['a', 'b', 'c', 'd', 'e', 'f', 'g'].map((seg, i) => ({
        id: generateNodeId('led'),
        type: 'led',
        position: { x: baseX + 200, y: baseY + (i * 25) },
        data: { label: `Seg ${seg.toUpperCase()}`, state: false },
      })),
      // Binary display showing input value
      {
        id: generateNodeId('binaryDisplay'),
        type: 'binaryDisplay',
        position: { x: baseX - 200, y: baseY + 200 },
        data: { label: 'BCD Input', bitWidth: 4 },
      }
    ];

    const newEdges = [
      // Connect BCD inputs to decoder
      ...Array.from({ length: 4 }, (_, i) => ({
        id: generateEdgeId(newNodes[i].id, newNodes[4].id, 'out', `in${i}`),
        source: newNodes[i].id,
        target: newNodes[4].id,
        sourceHandle: 'out',
        targetHandle: `in${i}`
      })),
      // Connect decoder outputs to segment LEDs (simplified - would need complex logic for real 7-segment)
  ...
  // Create binary display example circuit
  const addBinaryDisplayExample = useCallback(() => {
    const baseX = 250;
    const baseY = 200;
    
    const newNodes = [
      // 4 switches for 4-bit input
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 150, y: baseY },
        data: { label: 'Bit 3 (8)', state: true },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 150, y: baseY + 40 },
        data: { label: 'Bit 2 (4)', state: false },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 150, y: baseY + 80 },
        data: { label: 'Bit 1 (2)', state: true },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 150, y: baseY + 120 },
        data: { label: 'Bit 0 (1)', state: false },
      },
      // Binary display component
      {
        id: generateNodeId('binaryDisplay'),
        type: 'binaryDisplay',
        position: { x: baseX + 50, y: baseY + 60 },
        data: { label: '4-bit Display', bits: 4 },
      },
    ];

    const newEdges = [
      // Connect switches to binary display
      {
        id: generateEdgeId(newNodes[0].id, newNodes[4].id, 'out', 'bit0'),
        source: newNodes[0].id,
        target: newNodes[4].id,
        sourceHandle: 'out',
        targetHandle: 'bit0'
      ...
      {
        id: generateEdgeId(newNodes[1].id, newNodes[4].id, 'out', 'bit1'),
      ...
      {
        id: generateEdgeId(newNodes[2].id, newNodes[4].id, 'out', 'bit2'),
      ...
      {
        id: generateEdgeId(newNodes[3].id, newNodes[4].id, 'out', 'bit3'),
  ...
  // Create multi-LED example circuit
  const addMultiLEDExample = useCallback(() => {
    const baseX = 250;
    const baseY = 300;
    
    const newNodes = [
      // 6 switches for LED array
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY },
        data: { label: 'LED 0', state: true },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + 35 },
        data: { label: 'LED 1', state: false },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + 70 },
        data: { label: 'LED 2', state: true },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + 105 },
        data: { label: 'LED 3', state: true },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + 140 },
        data: { label: 'LED 4', state: false },
      },
      {
        id: generateNodeId('switch'),
        type: 'switch',
        position: { x: baseX - 200, y: baseY + 175 },
        data: { label: 'LED 5', state: true },
      },
      // Multi-LED component
      {
        id: generateNodeId('multiLED'),
        type: 'multiLED',
        position: { x: baseX + 50, y: baseY + 80 },
        data: { label: '6-LED Array', ledCount: 6 },
      },
    ];

    const newEdges = [
      // Connect switches to multi-LED
      {
        id: generateEdgeId(newNodes[0].id, newNodes[6].id, 'out', 'led0'),
        source: newNodes[0].id,
        target: newNodes[6].id,
        sourceHandle: 'out',
        targetHandle: 'led0'
      },
      {
        id: generateEdgeId(newNodes[1].id, newNodes[6].id, 'out', 'led1'),
        source: newNodes[1].id,
        target: newNodes[6].id,
        sourceHandle: 'out',
        targetHandle: 'led1'
      ...
      {
        id: generateEdgeId(newNodes[2].id, newNodes[6].id, 'out', 'led2'),
        source: newNodes[2].id,
        target: newNodes[6].id,
        sourceHandle: 'out',
        targetHandle: 'led2'
      ...
      {
        id: generateEdgeId(newNodes[3].id, newNodes[6].id, 'out', 'led3'),
        source: newNodes[3].id,
        target: newNodes[6].id,
        sourceHandle: 'out',
        targetHandle: 'led3'
      ...
      {
        id: generateEdgeId(newNodes[4].id, newNodes[6].id, 'out', 'led4'),
        source: newNodes[4].id,
        target: newNodes[6].id,
        sourceHandle: 'out',
        targetHandle: 'led4'
      ...
      {
        id: generateEdgeId(newNodes[5].id, newNodes[6].id, 'out', 'led5'),
        source: newNodes[5].id,
        target: newNodes[6].id,
        sourceHandle: 'out',
        targetHandle: 'led5'
      }
          ...
          <div className="sidebar" ref={sidebarRef} style={{ width: `${sidebarWidth}px` }}>
            <div className="sidebar-section">
              ...
              <div className="gates-container">
                ...
                <button onClick={addBinaryCounter} className="advanced-btn">Add Binary Counter                </button>
              </div>
            </div>
              <div className="sidebar-section">
              <h3>
              ...
              <div className="gates-container">
                ...
                <button onClick={() => addNode('binaryDisplay')} className="gate-btn">
                ...
                <button onClick={() => addNode('multiLED')} className="gate-btn">
                  <div className="gate-icon">🔴</div>
                  Multi LED
                </button>
            ...
            <div className="sidebar-section">
              <h3>
                ...
                <button onClick={addALU} className="example-btn">
                  <div className="gate-icon">🧮</div>
                  ALU Circuit
                </button>
                ...
                <button onClick={addBinaryCounter} className="example-btn">
                  <div className="gate-icon">📊</div>
                  8-bit Counter
                </button>
                ...
                <button onClick={add7SegmentDecoder} className="example-btn">
                  <div className="gate-icon">🔢</div>
                  7-Segment Decoder
                </button>                <button onClick={addDemultiplexer} className="example-btn">
                  <div className="gate-icon">🔀</div>
                  Demultiplexer
                </button>
                <button onClick={addBinaryDisplayExample} className="example-btn">
                  <div className="gate-icon">📟</div>
                  Binary Display
                </button>
                <button onClick={addMultiLEDExample} className="example-btn">
                  <div className="gate-icon">💡</div>
                  Multi-LED Array
                </button>
              </div>
